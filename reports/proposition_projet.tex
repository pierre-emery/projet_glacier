...
L'objectif principal est de construire un modèle d'apprentissage automatique capable de prédire l'évolution d'un glacier à partir de ses observations passées.
Concrètement, nous viserons quelque chose de relativement simple comme le changement d'aire par exemple du polygone qui représente l'outline du glacier.
...
De plus, le projet est aussi formateur et intéressant d'un point de vue apprentissage automatique:
\begin{itemize}
    \item Les données sont spatio-temporelles.
    \item Les obversations sont irrégulières dans le temps(pas de fréquence régulière).
    \item Les sources contiennent de l'incertitude (qualité variable des outlines), rendant la généralisation non-triviale.
\end{itemize} 
